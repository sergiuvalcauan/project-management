...
router.post('/:id/tes', function(req,res){
    ...
    db.collection('task').update( { '_id': ObjectID(taskInfo.TaskID)} , { $set: { 'Status': taskInfo.To }} )
    ...
    Project.findById( idProiect , function (err, project) {
        ...
        if (project) {
            // db.collection('project').update({ _id: project._id}, {$push: { tasks: { "id" : ObjectID(taskInfo.TaskID), "Status": taskInfo.To}} }, {upsert: false})
            db.collection('project').update({_id: project._id , "tasks.id": ObjectID(taskInfo.TaskID)}, {$set: {'tasks.$.Status': taskInfo.To}} )
        }
    });
